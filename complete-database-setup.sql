-- COMPLETE DATABASE SETUP FOR SPORTS VENUE BOOKING APP
...
-- First, drop the existing pricing_rules table if it exists with wrong columns
DROP TABLE IF EXISTS pricing_rules CASCADE;

-- Pricing rules table (optional)
...
FROM courts c
WHERE NOT EXISTS (
  SELECT 1 FROM pricing_rules pr WHERE pr.court_id = c.id AND pr.duration_hours = 1
);
...
FROM courts c
WHERE NOT EXISTS (
  SELECT 1 FROM pricing_rules pr WHERE pr.court_id = c.id AND pr.duration_hours = 2
);
